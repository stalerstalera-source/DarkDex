...
DefaultSettings = (function()
	...
	return {
		...
		AI = {
			_Recurse = true,
			ApiKey = "",
			ApiKeys = "sk-4deb7c21bc32424290dfb2dc127f4054", -- multiple keys, one per line
			DashScopeKey = "sk-4deb7c21bc32424290dfb2dc127f4054",
...
Main = (function()
	...
	Main.AI = (function()
		...
		local Models = {
			{
				Id = "qwen3.8-max",
				Label = "Qwen 3.8 Max (DashScope)",
				Provider = "openai",
				Endpoint = "https://dashscope.aliyuncs.com/compatible-mode/v1",
				DefaultKey = "sk-4deb7c21bc32424290dfb2dc127f4054",
			},
			{
				Id = "qwen3.8-flash",
				Label = "Qwen 3.8 Flash (DashScope Fast)",
				Provider = "openai",
				Endpoint = "https://dashscope.aliyuncs.com/compatible-mode/v1",
				DefaultKey = "sk-4deb7c21bc32424290dfb2dc127f4054",
			},
			...
			},
			{
				...
				Label = "Gemini 3.5 Flash Lite (Google Fast)",
				Provider = "gemini",
			},
			{
				Id = "custom-openai",
		...
		local function getModelInfo(id)
			for _, m in ipairs(Models) do
				if m.Id == id then return m end
			end
			if tostring(id):lower():find("gemini") then
				return {Id = id, Label = id, Provider = "gemini"}
			else
				return {
					Id = id,
					Label = id,
					Provider = "openai",
					Endpoint = Settings.AI.DashScopeEndpoint or "https://dashscope.aliyuncs.com/compatible-mode/v1",
					DefaultKey = "sk-4deb7c21bc32424290dfb2dc127f4054",
				}
			end
		end
		...
		local function buildOpenAITools()
			local decls = {}
			for name, tool in pairs(tools) do
				local fn = {
					name = name,
				...
				if tool.parameters and tool.parameters.properties and next(tool.parameters.properties) ~= nil then
...
- Reply in the same language as the user (Russian, English, Turkish, etc.). Keep responses tight; don't repeat tool results verbatim, summarize them.
- Your responses are rendered with markdown: **bold**, *italic*, `inline code`, and fenced ```code blocks``` all work. Use them to make output readable but don't over-format.]]
		...
		local function historyToOpenAI(history, systemPrompt)
			local messages = {}
			...
			for _, item in ipairs(history) do
				if item.role == "user" then
					table.insert(messages, {role = "user", content = item.text or ""})
				...
				elseif item.role == "tool" then
					for idx, tr in ipairs(item.toolResults or {}) do
						...
						table.insert(messages, {
							role = "tool",
							tool_call_id = tr.id or ("call_" .. idx),
							name = tr.name,
							content = contentStr,
						})
					end
		...
		local function historyToGemini(history)
			local contents = {}
			for _, item in ipairs(history) do
				if item.role == "user" then
					table.insert(contents, {role = "user", parts = {{text = item.text or ""}}})
				elseif item.role == "assistant" then
					local parts = {}
					if item.text and item.text ~= "" then
						table.insert(parts, {text = item.text})
					end
					if item.toolCalls then
						for _, tc in ipairs(item.toolCalls) do
							local fc = {name = tc.name}
							if tc.args and next(tc.args) ~= nil then
								fc.args = tc.args
							end
				...
				elseif item.role == "tool" then
					local parts = {}
					for _, tr in ipairs(item.toolResults or {}) do
						table.insert(parts, {
							functionResponse = {
								name = tr.name,
		...
		local function callOpenAI(modelInfo, history, apiKey)
			local endpoint = Settings.AI.DashScopeEndpoint
			if not endpoint or endpoint == "" then
				endpoint = modelInfo.Endpoint or "https://dashscope.aliyuncs.com/compatible-mode/v1"
			end
			endpoint = endpoint:gsub("/+$", "")
			local url = endpoint
			if not url:find("/chat/completions$") then
				url = url .. "/chat/completions"
			end

			local reqModel = modelInfo.Id
			if reqModel == "custom-openai" then
				reqModel = Settings.AI.CustomModel or "qwen3.8-max"
			end

			local messages = historyToOpenAI(history, SystemPrompt)
			local body = {
				model = reqModel,
				messages = messages,
				tools = buildOpenAITools(),
				temperature = 0.4,
				max_tokens = Settings.AI.MaxOutputTokens or 8192,
			}
			...
			if decoded.error then
				local msg = (typeof(decoded.error) == "table" and decoded.error.message) or tostring(decoded.error)
				return nil, "API error: " .. msg, tonumber(decoded.error.code)
			end

			return decoded
		end

		local function callModel(modelInfo, history, apiKey)
			if modelInfo.Provider == "openai" then
				local decoded, err, statusCode = callOpenAI(modelInfo, history, apiKey)
				if not decoded then return nil, err, statusCode end
				...
				local textOut = (msg.content and msg.content ~= "") and msg.content or nil
				local toolCalls = {}
				...
				if msg.tool_calls and #msg.tool_calls > 0 then
					for idx, tc in ipairs(msg.tool_calls) do
						...
						if fn and fn.name then
							...
							if not okArgs or typeof(parsedArgs) ~= "table" then parsedArgs = {} end
				...
				return {text = textOut, toolCalls = toolCalls}
			else
				local contents = historyToGemini(history)
				local decoded, err, statusCode = callGemini(modelInfo.Id, contents, apiKey)
				if not decoded then return nil, err, statusCode end
				...
				local textOut = #textParts > 0 and table.concat(textParts, "\n") or nil
				return {text = textOut, toolCalls = toolCalls}
			end
		end
		...
		local function getApiKeys(modelInfo)
			modelInfo = modelInfo or getModelInfo(Settings.AI.Model or "qwen3.8-max")
			local keys = {}
			...
			if modelInfo.Provider == "openai" then
				for _, k in ipairs(keys) do
					if k:sub(1,3) == "sk-" or not k:find("^AIza") then
						table.insert(filtered, k)
					end
				end
				if #filtered == 0 and modelInfo.DefaultKey then
					table.insert(filtered, modelInfo.DefaultKey)
				end
				if #filtered == 0 and Settings.AI.DashScopeKey and Settings.AI.DashScopeKey ~= "" then
					table.insert(filtered, Settings.AI.DashScopeKey)
				end
			else
				for _, k in ipairs(keys) do
					if not (k:sub(1,3) == "sk-") then
						table.insert(filtered, k)
					end
				end
			end
		...
		function AI.RunAgentLoop(userMessage)
			...
			local modelId = Settings.AI.Model or "qwen3.8-max"
			local modelInfo = getModelInfo(modelId)
			local apiKeys = getApiKeys(modelInfo)

			if #apiKeys == 0 then
				if modelInfo.Provider == "gemini" then
					addBubble("bot", "⚠ Please set your Gemini API key in the Settings panel (⚙ button).")
				else
					addBubble("bot", "⚠ Please set your API key in the Settings panel (⚙ button).")
				end
				AI.Running = false
				setStatus("")
				AI.UpdateSendButton()
			...
			while iterations < maxIter and not AI.SessionCancelled do
				...
				table.insert(AI.History, {role = "assistant", text = res.text or "", toolCalls = res.toolCalls})

				if res.text and res.text ~= "" then
					addBubble("bot", res.text)
				end

				if not res.toolCalls or #res.toolCalls == 0 then break end

				local respResults = {}
				for _, fc in ipairs(res.toolCalls) do
					if AI.SessionCancelled then break end
					local tool = tools[fc.name]
					local argStr = "{}"
					local okEnc, enc = pcall(httpService.JSONEncode, httpService, fc.args or {})
					if okEnc and enc then argStr = enc end
					addToolBubble(fc.name, argStr:sub(1,200))
					...
					table.insert(respResults, {id = fc.id, name = fc.name, result = result})
				end
				if AI.SessionCancelled then break end
				table.insert(AI.History, {role = "tool", toolResults = respResults})
				setStatus("Thinking...")
			end
		...
		function AI.Init()
			...
			keysLbl.Position = UDim2.new(0,14,0,82)
			...
			keysBox.Position = UDim2.new(0,14,0,98)
			keysBox.Size = UDim2.new(1,-28,0,95)
			keysBox.ZIndex = 11
			...
			-- Endpoint label & TextBox
			local endpointLbl = Instance.new("TextLabel", settingsPopup)
			endpointLbl.BackgroundTransparency = 1
			endpointLbl.Font = Enum.Font.GothamMedium
			endpointLbl.TextSize = 11
			endpointLbl.TextColor3 = ThemeSubtle
			endpointLbl.TextXAlignment = Enum.TextXAlignment.Left
			endpointLbl.Position = UDim2.new(0,14,0,198)
			endpointLbl.Size = UDim2.new(1,-28,0,14)
			endpointLbl.Text = "ENDPOINT (OpenAI / DashScope)"
			endpointLbl.ZIndex = 11

			local endpointBox = Instance.new("TextBox", settingsPopup)
			endpointBox.BackgroundColor3 = ThemeInputBg
			endpointBox.BorderSizePixel = 0
			endpointBox.Font = Enum.Font.Code
			endpointBox.TextSize = 11
			endpointBox.TextColor3 = ThemeText
			endpointBox.PlaceholderText = "https://dashscope.aliyuncs.com/compatible-mode/v1"
			endpointBox.PlaceholderColor3 = Color3.fromRGB(100,100,110)
			endpointBox.TextXAlignment = Enum.TextXAlignment.Left
			endpointBox.ClearTextOnFocus = false
			endpointBox.Position = UDim2.new(0,14,0,214)
			endpointBox.Size = UDim2.new(1,-28,0,22)
			endpointBox.ZIndex = 11
			local epCorner = Instance.new("UICorner", endpointBox)
			epCorner.CornerRadius = UDim.new(0,6)
			local epPad = Instance.new("UIPadding", endpointBox)
			epPad.PaddingLeft = UDim.new(0,8)
			epPad.PaddingRight = UDim.new(0,8)

			-- Preload existing keys & endpoint
			...
			if existingKeys == "" then
				existingKeys = "sk-4deb7c21bc32424290dfb2dc127f4054"
			end
			keysBox.Text = existingKeys
			endpointBox.Text = Settings.AI.DashScopeEndpoint or "https://dashscope.aliyuncs.com/compatible-mode/v1"
			...
			saveBtn.Position = UDim2.new(0,14,0,364)
			saveBtn.Size = UDim2.new(1,-28,0,28)
			saveBtn.ZIndex = 11
			...
			saveBtn.MouseButton1Click:Connect(function()
				...
				Settings.AI.DashScopeEndpoint = endpointBox.Text
			...
			settingsBtn.MouseButton1Click:Connect(function()
				...
				if settingsPopup.Visible then
					...
					if existKeys == "" then
						existKeys = "sk-4deb7c21bc32424290dfb2dc127f4054"
					end
			...
			local INPUT_MAX_H = 100
			local INPUT_BAR_PAD = 16 -- top+bottom padding around inputBox in the bar
			...
			-- Dynamically resize inputBar & chatFrame based on inputBox height
			local function updateInputBarSize()
				...
				-- Keep send button vertically centered
			...
			addBubble("bot", "Hi — I'm your Dex Copilot. Ask me anything about this game's DataModel, scripts, or instances. Open Settings (⚙) to switch models or configure API keys.")
		end
